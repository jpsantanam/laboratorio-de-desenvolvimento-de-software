document.addEventListener('DOMContentLoaded', () => {
    ...
    companyForm.addEventListener('submit', async (event) => {
        ...
        if (newPasswordInput.value.trim() || confirmNewPasswordInput.value.trim()) {
            ...
            if (passwordInput.value.trim() !== companyData.password) {
        ...
        try {
            // PUT request to update company account; adjust the endpoint as needed.
            const response = await fetch(`http://localhost:3000/companies/${companyData.id}`, {
                method: 'PUT',
            ...
            if (response.ok) {
                ...
                localStorage.setItem('company', JSON.stringify(dataFromServer));
                companyData = dataFromServer;
                enableViewMode();
            } else {
    ...
    deleteAccountButton.addEventListener('click', () => {
        const deleteModalElement = document.getElementById('deleteCompany');
        const deleteModal = new bootstrap.Modal(deleteModalElement);
        deleteModal.show();
    });

    document.getElementById('confirm-delete-company-form').addEventListener('submit', async (e) => {
        e.preventDefault();
